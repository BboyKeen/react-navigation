...
const BaseNavigationContainer = /*#__PURE__*/React.forwardRef(function BaseNavigationContainer(_ref, ref) {
  ...
  const onInitialRouteRef = React.useRef(onInitialRoute);
  const stateRef = React.useRef(state);
  React.useEffect(() => {
    isInitialRef.current = false;
    onStateChangeRef.current = onStateChange;
    onInitialRouteRef.current = onInitialRoute;
  ...
  React.useEffect(() => {
    ...
    if (isFirstMountRef.current && onInitialRouteRef.current) {
      onInitialRouteRef.current(hydratedState);
    }
